 ...
 *
 * On Railway:
 *   railway run node backend/scripts/seed-demo.js
 */
...
function calcPaystubTaxes({ gross, ppy = 26, ytdBefore = 0, suiRate = 0.027 }) {
  ...
  const suiBase  = 9000;
  const suiTaxable = Math.max(0, Math.min(gross, suiBase - Math.min(ytdBefore, suiBase)));
...
if (!adminUser) {
  console.error('❌  No admin user found. Make sure the app has been started at least once.');
  process.exit(1);
}
const adminId = adminUser.id;
console.log(`✓  Found admin user (id=${adminId})`);

// ── Demo companies definition ─────────────────────────────────────────────────
...
// ── Pay period sequences (Jan–May 2026 = 5 periods of history) ────────────────
...
function buildSemimonthlyPeriods(anchorEnd) {
  // Periods ending on 15th and last day of month
  const periods = [];
  let e = new Date(anchorEnd + 'T00:00:00');
  for (let i = 0; i < 10; i++) {
    let s;
    ...
    periods.unshift({ start: s.toISOString().slice(0,10), end: e.toISOString().slice(0,10) });
    // Move to previous period end
    if (e.getDate() <= 15) {
      const prev = new Date(e.getFullYear(), e.getMonth(), 0); // last day of prev month
      e = prev;
    } else {
...
const seedAll = db.transaction(() => {
  ...
  for (const co of COMPANIES) {
    // Idempotency check
    const existing = db.prepare("SELECT id FROM clients WHERE user_id=? AND business_name=?").get(adminId, co.business_name);
    if (existing) {
      console.log(`  ↩  Skipping "${co.business_name}" — already seeded`);
      continue;
    }

    // 1. Create company
    const clientRes = insertClient.run(adminId, co.business_name, co.ein, co.state,
      co.contact_name, co.contact_email, co.sui_rate, co.deposit_schedule);
    const clientId = clientRes.lastInsertRowid;
    console.log(`  ✓  Created company "${co.business_name}" (id=${clientId})`);

    // 2. Create pay groups
    const pg1Res = insertPayGroup.run(clientId, co.payGroup.name, co.payGroup.frequency,
      co.payGroup.anchorStart, co.payGroup.anchorEnd, co.payGroup.payDate);
    const pgId1 = pg1Res.lastInsertRowid;

    const pg2Res = insertPayGroup.run(clientId, co.payGroup2.name, co.payGroup2.frequency,
      co.payGroup2.anchorStart, co.payGroup2.anchorEnd, co.payGroup2.payDate);
    const pgId2 = pg2Res.lastInsertRowid;
    console.log(`  ✓  Created pay groups`);
    ...
    for (const e of co.employees) {
      ...
      const res = insertEmployee.run(clientId, e.first_name, e.last_name, e.pay_type,
        e.hourly_rate || 0, e.annual_salary || 0, e.filing_status, freq, pgId);
      empIds[`${e.first_name} ${e.last_name}`] = { id: res.lastInsertRowid, ...e, pgId, freq };
    }
    console.log(`  ✓  Created ${co.employees.length} employees`);

    // 4. Generate paystubs (printed history)
    for (const [name, emp] of Object.entries(empIds)) {
      ...
      for (const p of hist) {
        ...
        const settleDate = addBizDays(p.end + 'T00:00:00', 2);

        insertPaystub.run(
          clientId, emp.id, name,
          p.start, p.end, settleDate, freq,
    ...
    console.log(`  ✓  Seeded paystub history`);
  }
});

console.log('\n🌱  Seeding demo data...\n');
seedAll();
console.log('\n✅  Done! Demo companies are live on the admin account.\n');
console.log('   Login: admin / admin123');
console.log('   Companies: Austin Eats LLC [DEMO], Lone Star Staffing Inc. [DEMO]\n');
