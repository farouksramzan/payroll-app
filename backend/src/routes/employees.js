...
router.post('/', (req, res) => {
  const db = getDb();
  const { clientId, firstName, lastName, ssn, address, city, state, zip,
  ...
  const result = db.prepare(`
    INSERT INTO employees (client_id, first_name, last_name, ssn_encrypted, address, city, state, zip,
...
router.put('/:id', (req, res) => {
  ...
  const { firstName, lastName, ssn, address, city, state, zip,
    filingStatus, step2Checkbox, step3Children, step3Other, step4a, step4b, step4c,
    payType, hourlyRate, annualSalary, payFrequency, hireDate, isActive } = req.body;

  db.prepare(`
    UPDATE employees SET
      first_name=?, last_name=?, ssn_encrypted=?, address=?, city=?, state=?, zip=?,
