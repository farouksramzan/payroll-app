...
router.post('/print-selected', (req, res) => {
  const { clientId, paystubIds, design = 'classic' } = req.body;
  ...
  res.setHeader('Content-Disposition', `attachment; filename="checks-${design}.pdf"`);
  doc.pipe(res);

  for (const stub of stubs) {
    if (design === 'micr') {
      renderMICRCheck(doc, stub, client, routingNumber, accountNumber, db);
    } else if (design === 'top') {
      renderTopCheck(doc, stub, client, routingNumber, accountNumber, db);
    } else {
      renderClassicCheck(doc, stub, client, db);
    }
  }
...
function renderClassicCheck(doc, stub, client, db) {
...
function renderMICRCheck(doc, stub, client, routingNumber, accountNumber, db) {
  ...
  const CY = 528, CH = 264, CX = 36, CW = 540;
  const payDate = stub.settlement_date || stub.pay_period_end || new Date().toISOString().slice(0, 10);

  // Top border of check
  doc.rect(CX - 4, CY, CW + 8, 1).fill(BORDER);

  // Company block (upper-left)
  doc.font('Helvetica-Bold').fontSize(10).fillColor(DARK).text(client.business_name, CX, CY + 10, { width: CW / 2 });
  const companyCity2 = [client.business_city, client.state, client.business_zip].filter(Boolean).join(', ');
  let companyYOffset = CY + 22;
  if (client.business_address) { doc.font('Helvetica').fontSize(7.5).fillColor(GRAY).text(client.business_address, CX, companyYOffset, { width: CW / 2 }); companyYOffset += 10; }
  if (companyCity2) { doc.font('Helvetica').fontSize(7.5).fillColor(GRAY).text(companyCity2, CX, companyYOffset, { width: CW / 2 }); }

  // Check number (upper-right)
  doc.font('Helvetica-Bold').fontSize(11).fillColor(DARK)
    .text(stub.check_number ? `#${stub.check_number}` : '', CX + CW - 80, CY + 10, { width: 80, align: 'right' });

  // Date (right side, below check#)
  doc.font('Helvetica').fontSize(8).fillColor(GRAY)
    .text(`DATE: ${payDate}`, CX + CW - 150, CY + 28, { width: 150, align: 'right' });

  // PAY TO THE ORDER OF line (y = CY+58)
  doc.font('Helvetica').fontSize(8).fillColor(GRAY).text('PAY TO THE ORDER OF', CX, CY + 58);
  doc.font('Helvetica-Bold').fontSize(11).fillColor(DARK).text(empName, CX, CY + 68, { width: CW - 130 });
  // Horizontal line under name
  doc.rect(CX, CY + 84, CW - 130, 1).fill(BORDER);
  // Amount box (right side)
  doc.rect(CX + CW - 120, CY + 60, 120, 28).stroke(BORDER);
  doc.font('Helvetica-Bold').fontSize(12).fillColor(DARK)
    .text(`$${Number(stub.net_pay).toFixed(2)}`, CX + CW - 118, CY + 67, { width: 114, align: 'right' });

  // Amount in words (y = CY+100)
  doc.rect(CX, CY + 100, CW, 20).stroke(BORDER);
  doc.font('Helvetica').fontSize(9).fillColor(DARK)
    .text(numberToWords(stub.net_pay) + '****', CX + 4, CY + 106, { width: CW - 8 });

  // Memo line (y = CY+130)
  const memo = `Pay period: ${stub.pay_period_start} to ${stub.pay_period_end}`;
  doc.font('Helvetica').fontSize(8).fillColor(GRAY).text('MEMO:', CX, CY + 130);
  doc.fillColor(DARK).text(memo, CX + 38, CY + 130);

  // Signature line (y = CY+155)
  doc.rect(CX + CW - 180, CY + 155, 180, 1).fill(DARK);
  doc.font('Helvetica').fontSize(7).fillColor(GRAY)
    .text('Authorized Signature', CX + CW - 180, CY + 159, { width: 180, align: 'center' });

  // MICR line (y = CY+CH-28 = 764)
  const paddedCheck = String(stub.check_number || '').padStart(4, '0');
  const micrLine = routingNumber && accountNumber
    ? `⑆${routingNumber}⑆${accountNumber}⑈${paddedCheck}⑈`
    : `⑆         ⑆              ⑈${paddedCheck}⑈`;
  doc.font('Helvetica').fontSize(6).fillColor(GRAY).text('MICR LINE', CX, CY + CH - 42, { width: CW, align: 'center' });
  doc.font('Courier-Bold').fontSize(14).fillColor('#000')
    .text(micrLine, CX, CY + CH - 28, { width: CW, align: 'center' });

  // Outer border
  doc.rect(CX - 4, CY, CW + 8, CH).stroke(BORDER);
}
...
function renderTopCheck(doc, stub, client, routingNumber, accountNumber, db) {
  ...
  // ── Top 1/3: Check section (y 0–264) ─────────────────────────────────────────
  const CY = 0, CH = 264, CX = 36, CW = 540;
  const payDate = stub.settlement_date || stub.pay_period_end || new Date().toISOString().slice(0, 10);
  ...
  // MICR line at bottom of check section (y = 236)
  const paddedCheck = String(stub.check_number || '').padStart(4, '0');
  const micrLine = routingNumber && accountNumber
    ? `⑆${routingNumber}⑆${accountNumber}⑈${paddedCheck}⑈`
    : `⑆         ⑆              ⑈${paddedCheck}⑈`;
  doc.font('Helvetica').fontSize(6).fillColor(GRAY).text('MICR LINE', CX, 222, { width: CW, align: 'center' });
  doc.font('Courier-Bold').fontSize(14).fillColor('#000')
    .text(micrLine, CX, 236, { width: CW, align: 'center' });

  // Outer border
  doc.rect(CX - 4, CY, CW + 8, CH).stroke(BORDER);

  // ── Bottom 2/3: Paystub section (y 264–792) ──────────────────────────────────
  const STUB_TOP = 264;
  doc.rect(ML, STUB_TOP + 10, TW, 56).fill(ACCENT);
  doc.fillColor('#fff').font('Helvetica-Bold').fontSize(13)
    .text(client.business_name, ML + 10, STUB_TOP + 20, { width: TW / 2 });
  const companyAddrLine = [client.business_address, [client.business_city, client.state, client.business_zip].filter(Boolean).join(', ')].filter(Boolean).join(', ');
  doc.font('Helvetica').fontSize(7.5).text(companyAddrLine || `EIN: ${client.ein}`, ML + 10, STUB_TOP + 36);
  if (companyAddrLine) doc.font('Helvetica').fontSize(7.5).text(`EIN: ${client.ein}`, ML + 10, STUB_TOP + 46);
  doc.font('Helvetica-Bold').fontSize(9)
    .text('PAY STUB — DETACH AND RETAIN', ML + 10, companyAddrLine ? STUB_TOP + 37 : STUB_TOP + 43, { align: 'right', width: TW - 10 });
...
// ── POST /api/paystubs/paystub-selected — paystub-only PDF (no check section) ─
router.post('/paystub-selected', (req, res) => {
  ...
  res.setHeader('Content-Type', 'application/pdf');
  res.setHeader('Content-Disposition', 'attachment; filename="selected-paystubs.pdf"');
  doc.pipe(res);
  ...
  const PAY_LABELS = { regular: 'Regular Pay', salary: 'Salary', overtime: 'Overtime (×1.5)', tips: 'Reported Tips', bonus: 'Bonus', commission: 'Commission' };
  ...
  for (const stub of stubs) {
    ...
    doc.fillColor('#fff').font('Helvetica-Bold').fontSize(14).text(client.business_name || '—', 52, 50, { width: W - 130 });
    doc.font('Helvetica').fontSize(8).text(`EIN: ${client.ein || '—'}`, 52, 68);
    doc.font('Helvetica-Bold').fontSize(10).text('EARNINGS STATEMENT', 52, 80, { align: 'right', width: W - 12 });

    let y = 108;
    doc.rect(40, y, W, 1).fill(BORDER); y += 8;

    function kv(label, value, x, yy, width) {
      const w = width || 120;
      doc.font('Helvetica').fontSize(7).fillColor(GRAY).text(label.toUpperCase(), x, yy, { width: w });
      doc.font('Helvetica-Bold').fontSize(9).fillColor(DARK).text(value || '—', x, yy + 9, { width: w });
    }
    const col = W / 4;
    kv('Pay Period', `${fmtD(stub.pay_period_start)} – ${fmtD(stub.pay_period_end)}`, 40, y, col * 2 - 10);
    kv('Pay Date', fmtD(stub.settlement_date || stub.pay_period_end), 40 + col * 2, y);
    kv('Check #', stub.check_number ? `#${stub.check_number}` : '—', 40 + col * 3, y);
    y += 30;

    doc.rect(40, y, W, 1).fill(BORDER); y += 8;
    doc.font('Helvetica-Bold').fontSize(10).fillColor(DARK).text(empName, 40, y);
    y += 14;
    const addrParts = [stub.emp_address, [stub.emp_city, stub.emp_state, stub.emp_zip].filter(Boolean).join(', ')].filter(Boolean);
    if (addrParts.length) { doc.font('Helvetica').fontSize(8).fillColor(GRAY).text(addrParts.join('\n'), 40, y); y += addrParts.length * 11 + 4; }
    doc.rect(40, y, W, 1).fill(BORDER); y += 10;

    // Earnings section header
    doc.rect(40, y, W, 16).fill('#f3f4f6');
    doc.font('Helvetica-Bold').fontSize(7).fillColor(GRAY).text('EARNINGS', 44, y + 5);
    y += 16;

    // Column definitions
    const C = [
      { x: 44,              w: W * 0.40, label: 'Description', align: 'left'  },
      { x: 44 + W * 0.40,  w: W * 0.15, label: 'Hours',       align: 'right' },
      { x: 44 + W * 0.55,  w: W * 0.20, label: 'Rate',        align: 'right' },
      { x: 44 + W * 0.75,  w: W * 0.25, label: 'Amount',      align: 'right' },
    ];
    doc.rect(40, y, W, 14).fill('#f9fafb');
    doc.font('Helvetica-Bold').fontSize(7.5).fillColor(GRAY);
    C.forEach(c => doc.text(c.label, c.x, y + 3, { width: c.w - 4, align: c.align }));
    y += 14;

    let shade = false;
    const rows = lineItems.length > 0 ? lineItems : [{ pay_type: 'gross', description: 'Gross Wages', hours: null, rate: null, amount: stub.gross_wages }];
    for (const li of rows) {
      if (shade) doc.rect(40, y, W, 14).fill('#fafafa');
      doc.font('Helvetica').fontSize(8.5).fillColor(DARK);
      const vals = [
        PAY_LABELS[li.pay_type] || li.description || li.pay_type || 'Pay',
        li.hours != null ? String(li.hours) : '',
        li.rate  != null ? fmtM(li.rate)   : '',
        fmtM(li.amount),
      ];
      C.forEach((c, i) => doc.text(vals[i], c.x, y + 2, { width: c.w - 4, align: c.align }));
      y += 14; shade = !shade;
    }
    if (stub.bonus > 0)         { doc.font('Helvetica').fontSize(8.5).fillColor(DARK).text('Bonus',         C[0].x, y + 2, { width: C[0].w - 4 }).text(fmtM(stub.bonus),         C[3].x, y + 2, { width: C[3].w - 4, align: 'right' }); y += 14; }
    if (stub.commission > 0)    { doc.font('Helvetica').fontSize(8.5).fillColor(DARK).text('Commission',    C[0].x, y + 2, { width: C[0].w - 4 }).text(fmtM(stub.commission),    C[3].x, y + 2, { width: C[3].w - 4, align: 'right' }); y += 14; }
    if (stub.reimbursement > 0) { doc.font('Helvetica').fontSize(8.5).fillColor(DARK).text('Reimbursement', C[0].x, y + 2, { width: C[0].w - 4 }).text(fmtM(stub.reimbursement), C[3].x, y + 2, { width: C[3].w - 4, align: 'right' }); y += 14; }

    doc.rect(40, y, W, 16).fill('#eff6ff');
    doc.font('Helvetica-Bold').fontSize(9).fillColor(BLUE)
      .text('Gross Pay', 44, y + 4, { width: W * 0.75 - 4 })
      .text(fmtM(stub.gross_wages), 44 + W * 0.75, y + 4, { width: W * 0.25 - 4, align: 'right' });
    y += 16; y += 10;

    // Deductions — two-column mini sections
    const halfW = (W - 12) / 2;
    function miniSection(label, sRows, x, startY, width) {
      doc.rect(x, startY, width, 14).fill('#f3f4f6');
      doc.font('Helvetica-Bold').fontSize(7).fillColor(GRAY).text(label.toUpperCase(), x + 4, startY + 4, { width: width - 8 });
      let ry = startY + 14;
      sRows.forEach(([desc, amt], idx) => {
        if (idx % 2 === 1) doc.rect(x, ry, width, 13).fill('#fafafa');
        doc.font('Helvetica').fontSize(8.5).fillColor(DARK)
          .text(desc, x + 4, ry + 2, { width: width * 0.65 - 4 })
          .text(amt,  x + 4 + width * 0.65, ry + 2, { width: width * 0.35 - 8, align: 'right' });
        ry += 13;
      });
      return ry;
    }
    ...
    const empDedRows = [
      ['Federal Income Tax', fmtM(stub.fit_withholding)],
      ['Social Security',    fmtM(stub.employee_ss)],
      ['Medicare',           fmtM(stub.employee_medicare)],
      ...(stub.additional_medicare > 0 ? [['Additional Medicare', fmtM(stub.additional_medicare)]] : []),
    ...
    const erTaxRows = [
      ...
      ['Medicare Match (1.45%)', fmtM(stub.employer_medicare)],
      ...(stub.futa_tax > 0 ? [['FUTA (0.6%)',               fmtM(stub.futa_tax)]] : []),
      ...(stub.suta_tax > 0 ? [[`SUI — ${stub.work_state || ''}`, fmtM(stub.suta_tax)]] : []),
    ];

    const lBottom = miniSection('EMPLOYEE DEDUCTIONS', empDedRows, 40,              y, halfW);
    const rBottom = miniSection('EMPLOYER TAXES',      erTaxRows,  40 + halfW + 12, y, halfW);
    y = Math.max(lBottom, rBottom) + 10;
    ...
    doc.rect(40 + W * 0.35, y - 2, W * 0.65, 24).fill(ACCENT);
    doc.font('Helvetica-Bold').fontSize(12).fillColor('#fff')
      .text('NET PAY', 44 + W * 0.35, y + 4, { width: W * 0.65 * 0.5 })
      .text(fmtM(stub.net_pay), 44 + W * 0.35, y + 4, { width: W * 0.65 * 0.5 - 8, align: 'right' }); y += 26;
    doc.font('Helvetica').fontSize(8.5).fillColor(GRAY).text('EFTPS 941 Tax Deposit:', 40, y + 2, { width: W * 0.65 });
    doc.font('Helvetica-Bold').fillColor(DARK).text(fmtM(stub.total_deposit), 40, y + 2, { width: W, align: 'right' }); y += 18;

    // Footer
    doc.rect(40, y, W, 1).fill(BORDER); y += 6;
    doc.font('Helvetica').fontSize(7).fillColor(GRAY)
      .text(
        `Generated ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} · Paystub #${stub.id} · ${client.business_name || ''}`,
  ...
  const results = db.transaction(() => {
    ...
    for (const empData of employees) {
      ...
      created.push({
        ...
        directDeposit: emp.bank_account_status === 'active',
        moovAccountId: emp.moov_account_id || null,
        moovBankAccountId: emp.moov_bank_account_id || null,
      });
    }
      ...
      for (const stub of ddResults) {
        try {
          ...
          await moov.sendDirectDeposit({
            ...
            sourcePaymentMethodId: srcMethodId,
            destAccountId:        stub.moovAccountId,
            destPaymentMethodId:  destMethod.paymentMethodID,
            netPayCents:          amountCents,
            description:          `Payroll ${runId}`,
          });

          db2.prepare("UPDATE paystubs SET check_status = 'direct_deposit_sent' WHERE id = ?").run(stub.id);
          console.log(`[DD] Sent $${stub.netPay} to ${stub.employeeName} (stub ${stub.id})`);
        } catch (err) {
          console.error(`[DD] Failed for stub ${stub.id} (${stub.employeeName}):`, err.message);
        }
      }
    });
  }
