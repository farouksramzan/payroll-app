    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-secondary)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        {clientInfo?.businessName && (
          <>
            ...
            <div style={{ fontSize: '0.8667rem', fontWeight: 600, color: 'rgba(255,255,255,0.85)' }}>{clientInfo.businessName}</div>
          </>
        )}
        <div style={{ flex: 1 }} />
      ...
      <div style={{ flex: 1, maxWidth: 960, margin: '0 auto', width: '100%', padding: '28px 24px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h2 style={{ fontSize: '1.4667rem', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.3px', margin: 0 }}>Company Dashboard</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.8667rem', marginTop: 4 }}>Manage your employees and view payroll records.</p>
        </div>
        ...
        {tab === 'overview' && (
          <div className="card" style={{ padding: 0, overflowX: 'auto' }}>
            ...
            {paystubs.length === 0 ? (
              <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.9333rem' }}>
                ...
                <thead>
                  <tr style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)' }}>
                    {['Employee', 'Period', 'Gross Pay', 'Net Pay'].map(h => (
                      <th key={h} style={{ padding: '10px 18px', textAlign: 'left', fontSize: '0.7333rem', color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {paystubs.slice(0, 10).map(p => (
                    <tr key={p.id} style={{ borderBottom: '1px solid var(--border-light)' }}>
                      <td style={{ padding: '12px 18px', fontSize: '0.8667rem', fontWeight: 600 }}>{p.first_name} {p.last_name}</td>
                      <td style={{ padding: '12px 18px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Q{p.tax_quarter} {p.tax_year}</td>
                      <td style={{ padding: '12px 18px', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8667rem' }}>{fmt(p.gross_wages)}</td>
                      <td style={{ padding: '12px 18px', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8667rem', color: 'var(--accent)', fontWeight: 600 }}>{fmt(p.net_pay)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {clientInfo?.joinCode && (
              <div className="card" style={{ borderLeft: '4px solid var(--accent)', padding: '16px 20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                  <div>
                    <div style={{ fontSize: '0.9333rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>Employee Join Code</div>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', margin: '0 0 12px', maxWidth: 500 }}>
                ...
                employees.map(e => (
                  <div key={e.id} style={{ display: 'flex', alignItems: 'center', padding: '14px 20px', borderBottom: '1px solid var(--border-light)', gap: 14 }}>
                    <div style={{
                      ...
                      fontSize: '0.8667rem', fontWeight: 700, flexShrink: 0,
                    }}>
                      {(e.firstName?.[0] || '?')}{(e.lastName?.[0] || '')}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '0.9333rem', fontWeight: 600, color: 'var(--text-primary)' }}>{e.firstName} {e.lastName}</div>
        ...
        {tab === 'paystubs' && (
          <div className="card" style={{ padding: 0, overflowX: 'auto' }}>
            <div className="card-header" style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-light)' }}>
              ...
              <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{paystubs.length} records</span>
            </div>
            {paystubs.length === 0 ? (
              <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.9333rem' }}>
                No pay records yet.
                ...
                <tbody>
                  {paystubs.map(p => (
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 18px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>Q{p.tax_quarter} {p.tax_year}</td>
                      <td style={{ padding: '12px 18px', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8667rem' }}>{fmt(p.gross_wages)}</td>
                      <td style={{ padding: '12px 18px', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8667rem' }}>{fmt(p.fit_withholding)}</td>
                      <td style={{ padding: '12px 18px', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8667rem', color: 'var(--accent)', fontWeight: 600 }}>{fmt(p.net_pay)}</td>
                    </tr>
                  ))}
                </tbody>
...
function StatCard({ value, label, color, mono }) {
  return (
    <div className="card" style={{ textAlign: 'center', padding: '20px 16px' }}>
      <div style={{
        fontSize: '1.8667rem', fontWeight: 900,
