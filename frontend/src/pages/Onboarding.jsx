...
const inputStyle = {
  ...
  border: '1.5px solid var(--border)', fontSize: '0.9333rem',
  color: 'var(--text-primary)', background: 'var(--bg)',
  outline: 'none', boxSizing: 'border-box',
};
const labelStyle = {
  display: 'block', fontSize: '0.8667rem', fontWeight: 600,
...
function Field({ label, hint, children }) {
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {children}
      {hint && <div style={{ fontSize: '0.7333rem', color: 'var(--text-muted)', marginTop: 4 }}>{hint}</div>}
    </div>
  );
}
...
function PayGroupForm({ value, onChange, onSave, onCancel, saving, isFirst }) {
  ...
  return (
    <div style={{ background: 'var(--bg-secondary)', border: '1.5px solid var(--border)', borderRadius: 10, padding: 18 }}>
      <div style={{ fontSize: '0.8667rem', fontWeight: 700, color: 'var(--accent)', marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.4px' }}>
      ...
      <Field label="Pay Frequency *">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {Object.entries(FREQ_LABEL).map(([k, lbl]) => (
            ...
            >
              <div style={{ fontWeight: 700, fontSize: '0.8667rem', color: value.frequency === k ? 'var(--accent)' : 'var(--text-primary)' }}>{lbl}</div>
            </div>
          ))}
        </div>
      </Field>
      ...
      >
        ...
        {isWeekend && suggestedPD && (
          <div style={{ fontSize: '0.7333rem', color: '#d97706', marginTop: 4 }}>
            ⚠ Weekend or holiday —{' '}
            <button
              type="button"
              onClick={() => onChange('payDate', suggestedPD)}
              style={{ background: 'none', border: 'none', color: 'var(--accent)', fontWeight: 700, fontSize: '0.7333rem', cursor: 'pointer', padding: 0 }}
            >
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
        <button
          onClick={onSave}
          disabled={saving}
          style={{ flex: 1, padding: '10px 0', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '0.9333rem', cursor: 'pointer' }}
        >
        ...
        {onCancel && (
          <button
            onClick={onCancel}
            style={{ padding: '10px 16px', background: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1.5px solid var(--border)', borderRadius: 8, fontWeight: 600, fontSize: '0.8667rem', cursor: 'pointer' }}
          >
            Cancel
          </button>
        )}
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-secondary)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ height: 'var(--nav-h)', background: 'var(--accent)', display: 'flex', alignItems: 'center', padding: '0 24px' }}>
        <span style={{ color: '#fff', fontWeight: 800, fontSize: '1.1333rem', letterSpacing: '-0.3px' }}>PayrollTax Pro</span>
      </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.id} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '16px 8px', position: 'relative' }}>
                ...
                <div style={{ width: 28, height: 28, borderRadius: '50%', zIndex: 1, background: done || active ? 'var(--accent)' : 'var(--bg-tertiary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', fontWeight: 700, color: done || active ? '#fff' : 'var(--text-muted)' }}>
                  {done ? '✓' : s.id}
                </div>
                <div style={{ fontSize: '0.7333rem', marginTop: 6, color: active ? 'var(--accent)' : 'var(--text-muted)', fontWeight: active ? 700 : 400, textAlign: 'center' }}>
                  {s.label}
                </div>
              </div>
            );
      ...
      <div style={{ flex: 1, display: 'flex', justifyContent: 'center', padding: '40px 24px' }}>
        <div style={{ width: '100%', maxWidth: 580 }}>
          ...
          {step === 1 && (
            <div>
              <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: 4 }}>Tell us about your business</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9333rem', marginBottom: 28 }}>We'll use this for payroll documents and tax filings.</p>
              <div className="card" style={{ padding: 24 }}>
                ...
                {error && <div style={{ color: 'var(--error)', fontSize: '0.8667rem', marginBottom: 12 }}>{error}</div>}
                <button onClick={handleStep1} disabled={saving} style={{ width: '100%', padding: '12px 0', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '1rem', cursor: 'pointer', marginTop: 8 }}>
          ...
          {step === 2 && (
            <div>
              <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: 4 }}>Set up pay groups</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9333rem', marginBottom: 8 }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  {payGroups.map(g => (
                    <div key={g.id} className="card" style={{ padding: '14px 18px', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: '0.9333rem', color: 'var(--text-primary)' }}>{g.name}</div>
              ...
              {showAddForm ? (
                <div style={{ marginBottom: 16 }}>
                  {error && <div style={{ color: 'var(--error)', fontSize: '0.8667rem', marginBottom: 10 }}>{error}</div>}
                  <PayGroupForm
                    value={groupForm}
                ...
                <button
                  ...
                  style={{ width: '100%', padding: '11px 0', background: 'var(--bg)', border: '2px dashed var(--border)', borderRadius: 8, color: 'var(--accent)', fontWeight: 700, fontSize: '0.9333rem', cursor: 'pointer', marginBottom: 16 }}
                >
                  + Add Another Pay Group
                </button>
              )}

              {error && !showAddForm && <div style={{ color: 'var(--error)', fontSize: '0.8667rem', marginBottom: 12 }}>{error}</div>}

              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => { setStep(1); setError(''); }} style={{ flex: 1, padding: '12px 0', background: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1.5px solid var(--border)', borderRadius: 8, fontWeight: 600, fontSize: '0.9333rem', cursor: 'pointer' }}>
                  ← Back
                </button>
                <button
                  onClick={handleProceedToStep3}
                  disabled={payGroups.length === 0}
                  style={{ flex: 2, padding: '12px 0', background: payGroups.length === 0 ? 'var(--bg-tertiary)' : 'var(--accent)', color: payGroups.length === 0 ? 'var(--text-muted)' : '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '1rem', cursor: payGroups.length === 0 ? 'not-allowed' : 'pointer' }}
                >
          ...
          {step === 3 && (
            <div>
              <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: 4 }}>Invite your employees</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.9333rem', marginBottom: 28 }}>
                Share your company join code so employees can create their own accounts and view paystubs.
              </p>
              <div className="card" style={{ padding: 24 }}>
                <div style={{ textAlign: 'center', marginBottom: 28 }}>
                  <div style={{ fontSize: '0.8667rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                  ...
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 12, background: 'var(--bg-secondary)', border: '2px dashed var(--border)', borderRadius: 12, padding: '16px 24px' }}>
                    <span style={{ fontSize: '2.1333rem', fontWeight: 900, letterSpacing: '0.3em', fontFamily: 'JetBrains Mono, monospace', color: 'var(--accent)' }}>
                      {joinCode || '———'}
                    </span>
                    <button onClick={copyCode} style={{ padding: '8px 14px', borderRadius: 8, border: '1.5px solid var(--border)', background: copied ? 'var(--accent)' : 'var(--bg)', color: copied ? '#fff' : 'var(--text-primary)', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer', transition: 'all 0.15s' }}>
                      {copied ? '✓ Copied!' : 'Copy'}
                    </button>
                  </div>
                </div>
                ...
                <div style={{ background: 'var(--bg-secondary)', borderRadius: 10, padding: 20, marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                    <div style={{ background: 'var(--accent)', color: '#fff', borderRadius: 6, padding: '3px 10px', fontSize: '0.7333rem', fontWeight: 800, letterSpacing: '0.4px' }}>OPTION A</div>
                    <div style={{ fontSize: '0.8667rem', fontWeight: 700, color: 'var(--text-primary)' }}>Employees self-register with the join code</div>
                  </div>
                  {[
                    ...
                    <div key={item.n} style={{ display: 'flex', gap: 12, marginBottom: 8, alignItems: 'flex-start' }}>
                      <div style={{ minWidth: 22, height: 22, borderRadius: '50%', background: 'var(--accent)', color: '#fff', fontSize: '0.7333rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{item.n}</div>
                      <div style={{ fontSize: '0.8667rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>{item.text}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ background: 'var(--bg-secondary)', borderRadius: 10, padding: 20, marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                    <div style={{ background: 'var(--bg-tertiary)', color: 'var(--text-muted)', borderRadius: 6, padding: '3px 10px', fontSize: '0.7333rem', fontWeight: 800, letterSpacing: '0.4px', border: '1px solid var(--border)' }}>OPTION B</div>
                    <div style={{ fontSize: '0.8667rem', fontWeight: 700, color: 'var(--text-primary)' }}>You add employees manually from the dashboard</div>
                  </div>
                  {[
                    ...
                    <div key={item.n} style={{ display: 'flex', gap: 12, marginBottom: 8, alignItems: 'flex-start' }}>
                      <div style={{ minWidth: 22, height: 22, borderRadius: '50%', background: 'var(--bg-tertiary)', border: '1.5px solid var(--border)', color: 'var(--text-muted)', fontSize: '0.7333rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{item.n}</div>
                      <div style={{ fontSize: '0.8667rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>{item.text}</div>
                    </div>
                  ))}
                </div>
                ...
                {error && <div style={{ color: 'var(--error)', fontSize: '0.8667rem', marginBottom: 12 }}>{error}</div>}
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => { setStep(2); setError(''); }} style={{ flex: 1, padding: '12px 0', background: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1.5px solid var(--border)', borderRadius: 8, fontWeight: 600, fontSize: '0.9333rem', cursor: 'pointer' }}>
                    ← Back
                  </button>
                  <button onClick={handleStep3} disabled={saving} style={{ flex: 2, padding: '12px 0', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: '1rem', cursor: 'pointer' }}>
          ...
          {step === 4 && (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              <div style={{ fontSize: '3.7333rem', marginBottom: 16 }}>🎉</div>
              <h2 style={{ fontSize: '1.8667rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: 8 }}>You're all set!</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '1rem', maxWidth: 420, margin: '0 auto 32px' }}>
              ...
              <div className="card" style={{ padding: 28, maxWidth: 420, margin: '0 auto 24px', textAlign: 'left' }}>
                <div style={{ fontSize: '0.8667rem', fontWeight: 700, color: 'var(--text-muted)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>What's next</div>
                {[
                  ...
                  <div key={item.title} style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
                    <div style={{ fontSize: '1.3333rem', minWidth: 28 }}>{item.icon}</div>
                    <div>
                      <div style={{ fontSize: '0.9333rem', fontWeight: 600, color: 'var(--text-primary)' }}>{item.title}</div>
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: 2 }}>{item.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={() => navigate(`/company/${clientId}`, { replace: true })} style={{ padding: '14px 40px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 10, fontWeight: 800, fontSize: '1.0667rem', cursor: 'pointer' }}>
                Go to My Dashboard →
              </button>
            </div>
          )}
